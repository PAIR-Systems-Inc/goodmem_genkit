 ...
 *
 * Run from the repo root with `npx tsx examples/example_usage.ts`. To inspect
 * traces in the Genkit Developer UI, run `genkit start -- tsx
 * examples/example_usage.ts` and open http://localhost:4000.
 */

import { openAI } from '@genkit-ai/compat-oai/openai';
import { genkit } from 'genkit';
import { runInNewSpan } from 'genkit/tracing';
// In your own project, import the plugin by its published name:
//   import { goodmem } from 'genkitx-goodmem';
// This file uses a relative import so it can run from a checkout of this
// repo without a publish or link step.
import { goodmem } from '../src/index';
  ...
  if (process.env.GENKIT_ENV === 'dev') {
    ...
    console.log(
      '\n  Done. To inspect traces in the Genkit Developer UI, rerun ' +
        'with `genkit start -- tsx examples/example_usage.ts`. The UI ' +
        'will be at http://localhost:4000 while the process is alive.\n'
    );
  }
}
